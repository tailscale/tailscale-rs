...
use ts_keys::{Export, NodeKeyPair, NodePublicKey};
...
pub struct Args {
    ...
    pub peer_key: Export<NodeKeyPair>,

    /// Our private key. Can be hex or base64.
    #[clap(long, value_parser = parse_key)]
    pub private_key: NodeKeyPair,
}

type BoxResult<T> = Result<T, Box<dyn core::error::Error + Send + Sync>>;

/// Parse a [`Export<NodeKeyPair>`] from a string, trying hex (Tailscale-typical) and
/// base64 (WireGuard-typical) formats in succession.
pub fn parse_key(s: &str) -> BoxResult<NodeKeyPair> {
    ...
    Ok(Export::from_bytes(key_bytes).import())
}
...
async fn main() -> BoxResult<()> {
    ...
    eprintln!(
        "my pubkey: {}",
        base64::engine::general_purpose::STANDARD.encode(args.private_key.public.as_bytes())
    );

    let peer_key = *NodePublicKey::try_ref_from_bytes(args.peer_key.as_bytes())
        .map_err(|_| "failed reading public key")?;

    let mut ep = Endpoint::new(args.private_key);
