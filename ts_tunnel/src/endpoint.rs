...
impl Endpoint {
    ...
    /// # Panics
    ///
    /// If the [`PublicKey<Node>`] in the new [`PeerConfig`] collides with an existing key
    ...
    pub fn peer_key(&self, id: PeerId) -> Option<PublicKey<Node>> {
        let peer = self.peers.get(&id)?;
        Some(peer.config.key)
    }

    /// Return the peer id that has the selected node key.
    pub fn peer_id(&self, key: PublicKey<Node>) -> Option<PeerId> {
