//! Address parsing for `PF_ROUTE` messages.
//!
//! This module provides [`Address`], an enum which captures the `sockaddr_*` types we might expect
//! to find in `PF_ROUTE` messages.
//!
//! Addresses are laid out in the message after the
//! [`MessageHeader`][crate::bsd::net_table::MessageHeader] as back-to-back 4-byte-aligned
//! `sockaddr` entries. Because the sockaddr starts with `sa_len`, `sa_family`, this makes them
//! TLVs, so they can mostly be parsed in a context-free manner. The meaning of each [`Address`] is
//! indicated by the [`Addrs`][crate::bsd::net_table::Addrs`] in the message header; we don't need
//! to know that in order to parse the messages.
...
impl Address {
    ...
    /// zeroes and pads each possibly-truncated message up to 4-byte alignment. Also, it may null out
    /// the `sockaddr` completely (and truncate to _just_ `sa_len` and `sa_family`, plus padding).
    ///
    /// The `None` return means specifically that the address was of an unsupported `sockaddr`
    /// type; all failures are returned as errors, and situations implying the unspecified address
    /// return [`Address::Unspecified`].
    pub fn parse<I, E>() -> impl Parser<I, Output = Option<Self>, Error = E>
    where
                ...
                alt((
                    ...
                    .map(|(v4, v6)| {
                        Some(Address::PrefixLen(PrefixLen {
                            v4: v4.and_then(|addr| {
                                let pfx = netmask_to_prefix(&addr.into());
    ...
    (
        ...
        .map(|(kame_scope, (mut octets, _n), scope)| {
            ...
            if let Some(kame_scope) = kame_scope
                && kame_scope != 0
            {
                if scope != 0 && scope != kame_scope as u32 {
                    tracing::warn!(
...
/// Attempt to interpret `netmask` as a strict prefix, returning `Some` only if its set bits
/// are contiguous from the front.
///
/// Returns the prefix length if the mask is valid.
pub const fn netmask_to_prefix(netmask: &IpAddr) -> Option<u8> {
    let (count, leading) = match netmask {
        IpAddr::V4(ip) => {
            let x = ip.to_bits();
            (x.count_ones(), x.leading_ones())
        }
        IpAddr::V6(ip) => {
            let x = ip.to_bits();
            (x.count_ones(), x.leading_ones())
...
mod test {
    ...
    #[test]
    fn unspec_chunk() {
        assert_parse(&[0, 0, 0, 0], Address::parse(), Some(Address::Unspecified));
    }
    ...
    fn trailing_empty() {
        ...
        let expected = vec![
            ...
            // normally expected for RTA_BRD, but omitted because of the truncation behavior (see
            // note above):
            // Some(Address::Unspecified),
        ];

        assert_parse(SAMPLE, count(Address::parse(), expected.len()), expected);
    }
}
