...
async fn derp_receive_ping(derp: impl Borrow<ts_derp::DefaultClient>) {
    use bytes::Buf;
...
async fn derp_send_ping(peer: NodePublicKey, derp: impl Borrow<ts_derp::DefaultClient>) {
    use bytes::BufMut;

    let mut ticker = tokio::time::interval(Duration::from_secs(1));
    let derp = derp.borrow();
    let mut packet = [0u8; size_of::<u32>()];

    loop {
        let val = PING_MAX.fetch_add(1, core::sync::atomic::Ordering::SeqCst);
        (&mut packet[..]).put_u32(val);

        derp.send_one(peer, &packet).await.unwrap();
