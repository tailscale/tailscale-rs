...
impl<T: X25519Private> ReceivedHandshake<T> {
    ...
    pub fn new(packet: &mut [u8], prologue: &[u8], my_static: &KeyPair<T>) -> Option<Self> {
    ...
    fn finish_with_ephemeral(self, packet: &mut [u8], my_ephemeral: DalekKeyPair) -> Session {
...
pub struct SentHandshake<T: X25519Private> {
    state: State,
    my_ephemeral: x25519_dalek::StaticSecret,
    _marker: PhantomData<T>,
}

impl<T: X25519Private> SentHandshake<T> {
    ...
    /// attempt later.
    pub fn try_finish(self, packet: &mut [u8], my_static: &PrivateKey<T>) -> Result<Session, Self> {
        ...
        let ret = state
            ...
            .mix_dh(&my_static.into(), &peer_ephemeral_pub) // se
...
mod tests {
    ...
    fn test_key(r: Range<u8>) -> KeyPair<Node> {
        ...
        let private = ExportableKey::from(key).import();
    ...
    fn test_handshake() {
        ...
        let mut init_packet = [0; SentHandshake::<Node>::INIT_SIZE];
        ...
        let resp_session = resp_recv.finish_with_ephemeral(&mut resp_packet, resp_ephemeral.into());
