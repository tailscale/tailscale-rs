...
pub type EndpointStorage =
    smallbox::SmallBox<dyn Endpoint, [u8; size_of::<ts_keys::NodePublicKey>()]>;
...
impl Ord for DynEndpoint {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        // at the scope of any given compilation (`TypeId`s are assigned at compile-time).
        //
...
impl DynEndpoint {
    ...
    /// `CallMeMaybe` (and ought not send pings and pongs).
...
#[derive(Copy, Clone, PartialEq, Eq, Hash, PartialOrd, Ord)]
#[repr(transparent)]
pub struct DerpEndpoint(pub ts_keys::NodePublicKey);
