# ts_netstack_smoltcp::netsock

Ergonomic sockets API layer built around [`crate::netcore`].

The idea is to take the command [`Channel`][crate::netcore::Channel] and use
...
`T where T:` [`HasChannel`][crate::netcore::HasChannel].

## Example

Compare the example in [`crate::netcore`]:

```rust
use core::net::SocketAddr;
use bytes::Bytes;
use ts_netstack_smoltcp::{
    netcore::{Response, udp, HasChannel, self},
    CreateSocket,
    smoltcp::{time::Instant, phy::Medium, self},
};
...
fn main() {
    ...
    assert_eq!(
        packet.len(), 
        smoltcp::wire::IPV4_HEADER_LEN + smoltcp::wire::UDP_HEADER_LEN + b"hello".len(),
    );
