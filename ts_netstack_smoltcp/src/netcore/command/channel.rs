...
use crate::{
    netcore,
    netcore::{ChannelClosedError, Command, Error, Netstack, Response, command::Request},
};
...
pub trait HasChannel {
    ...
    ///
    /// Convenience wrapper around [`request_blocking`][netcore::request_blocking].
    fn request_blocking(
        &self,
        handle: Option<SocketHandle>,
        command: impl Into<Command>,
    ) -> Result<Response, ChannelClosedError> {
        netcore::request_blocking(self.borrow_channel().borrow(), handle, command)
    }

    /// Asynchronously send a request through the channel.
    ///
    /// Convenience wrapper around [`request`][netcore::request].
    fn request(
        &self,
        handle: Option<SocketHandle>,
        command: impl Into<Command>,
        ...
        async move { netcore::request(&ch, handle, command).await }
    }
    ...
    ///
    /// Convenience wrapper around [`request_nonblocking`][netcore::request_nonblocking].
    fn request_nonblocking(
        &self,
        handle: Option<SocketHandle>,
        command: impl Into<Command>,
    ) -> Result<(), ChannelClosedError> {
        netcore::request_nonblocking(self.borrow_channel().borrow(), handle, command)
    }
}
